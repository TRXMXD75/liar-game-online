...
// --- الأكشن العنيف للـ Pass والـ Bluff ---
...
function playSound(type) {
    ...
    try {
        ...
        if(type === 'play') { 
            osc.frequency.setValueAtTime(400, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(800, audioCtx.currentTime + 0.1); 
        ...
        else if(type === 'big_bluff') { 
            // صوت إنذار ضخم ومرعب للتكذيب
            osc.type = 'sawtooth'; osc.frequency.setValueAtTime(300, audioCtx.currentTime); osc.frequency.linearRampToValueAtTime(100, audioCtx.currentTime + 0.8); 
            gainNode.gain.setValueAtTime(0.5, audioCtx.currentTime); gainNode.gain.linearRampToValueAtTime(0.01, audioCtx.currentTime + 0.8); 
            osc.start(); osc.stop(audioCtx.currentTime + 0.8); 
        }
        else if(type === 'big_pass') { 
            // صوت صرخة رياح قوية للـ Pass
            osc.type = 'triangle'; osc.frequency.setValueAtTime(500, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(100, audioCtx.currentTime + 0.5); 
            gainNode.gain.setValueAtTime(0.4, audioCtx.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.5); 
            osc.start(); osc.stop(audioCtx.currentTime + 0.5); 
        }
        else if(type === 'win') { 
            osc.type = 'square'; osc.frequency.setValueAtTime(440, audioCtx.currentTime); osc.frequency.setValueAtTime(554, audioCtx.currentTime + 0.2); osc.frequency.setValueAtTime(659, audioCtx.currentTime + 0.4); 
            gainNode.gain.setValueAtTime(0.3, audioCtx.currentTime); gainNode.gain.linearRampToValueAtTime(0, audioCtx.currentTime + 1); 
...
let lobbyNames = []; // للاحتفاظ بأسماء المتواجدين في اللوبي
    ...
    c.on('data', data => {
        if(data.type === 'HELLO_HOST') {
            if(gameState.gameStarted) {
                ...
                if(existing) { 
                    c.playerName = data.name; c.playerToken = data.token; connections.push(c); 
        ...
        else if (data.type === 'BIG_EVENT') {
            // استقبال الأكشن الكبير
            playSound(data.sound);
            showBigAction(data.text, data.color);
            // تصفير الاختيار إذا وصلني الدور
            document.getElementById('claim-val').value = ""; 
        }
        else if (data.type === 'BACK_TO_LOBBY') {
            resetToLobby(data.names);
        }
    });
    c.on('close', () => { 
        connections = connections.filter(cli => cli !== c); 
        if(!gameState.gameStarted) broadcastLobby(); 
    });
}

function startHost() { 
    initAudio(); myName = getPlayerName(); isHost = true; 
    lobbyNames = [myName]; updateLobbyUI();
...
function joinRoom() { 
    initAudio(); myName = getPlayerName(); const hostId = document.getElementById('join-id').value.trim(); 
    if(!hostId) return showModal("تنبيه", "أدخل الكود!", "error"); 
    document.getElementById('initial-setup').innerHTML = "<h2>جاري البحث... ⏳</h2>"; 
    peer = new Peer(); 
    peer.on('open', id => { conn = peer.connect(hostId); setupConn(conn); conn.on('open', () => conn.send({ type: 'HELLO_HOST', name: myName, token: myToken })); }); 
...
function broadcastBigEvent(text, color, sound) {
    let eventData = { type: 'BIG_EVENT', text, color, sound };
    if(isHost) {
        playSound(sound); showBigAction(text, color); document.getElementById('claim-val').value = "";
        connections.forEach(c => { try { c.send(eventData); } catch(e){} });
    } else {
        conn.send(eventData); // Note: Since logic is distributed, usually Host sends it. But here we just show it locally if client and wait for sync.
        // Better: Just show locally for the triggerer, and triggerer tells host to broadcast?
        // Let's just trigger locally for me, and when I syncData, the others won't see the big event unless I send it.
    }
}
// Helper to send big event directly
function sendBigEventToAll(text, color, sound) {
    ...
    if(isHost) {
        ...
        playSound(sound); showBigAction(text, color); document.getElementById('claim-val').value = "";
        try { conn.send(eventData); } catch(e){} // Not perfectly robust for clients to broadcast, so clients will just rely on standard state update + local effect.
        // Actually, let's fix that. If client calls pass, we just need a way to tell everyone. I'll handle it inside sync.
    }
}
...
function callBluff() {
    ...
    setTimeout(() => {
        ...
        document.getElementById('claim-val').value = ""; 
        checkWin(); syncData();
    }, 2500); // ننتظر الأنيميشن يخلص بعدين نوزع الورق
}
...
// --- إعادة اللعب بدون Reload ---
function playAgain() {
    if(!isHost) return;
    gameState.gameStarted = false;
    broadcastLobby(); // نحدث اللوبي 
    resetToLobby(lobbyNames);
    // إرسال للكل للعودة
...
function render() {
    ...
    // التحكم باللوحة المضيئة للرقم (Hologram)
    ...
    if(gameState.currentClaim) { 
        ...
        document.getElementById('claim-target-number').innerText = gameState.currentClaim;
        claimHolo.style.display = "block";
    } else { 
        claimHolo.style.display = "none";
        if(gameState.currentPlayer === myIndex) { claimSelect.style.display = "inline-block"; } 
